...
def parse_arguments(argv):
    ...
    parser.add_argument('-s', '--save', default='/Users/emilybradley/Desktop/runstats/swimtraininggraph.html',
        help='directory location and name under which the graph will be saved, e.g. /Users/emilybradley/Desktop/runstats/swimtraininggraph.html')
...
def main(argv):
    ...
    logging.info('Building your swimming form graph!')

    try:
        swim_scores_df = pd.read_csv('swimscores.csv')
        #convert date column from string to pandas timestamp type
        swim_scores_df['date'] = pd.to_datetime(swim_scores_df['date'])
    except:
        logging.error("Couldn't find the csv file of swim scores in the current directory.")
        sys.exit(0)

    swim_dates = swim_scores_df['date'].tolist()
    ...
    day_count = (max(swim_dates)-min(swim_dates)).days

    if args.months == 'all':
        date_range = [min(swim_dates) + timedelta(n) for n in range(day_count)]
    ...
    mask = swim_scores_df['date'].isin(date_range)
    swim_scores_to_plot = swim_scores_df[mask]
    swim_scores_to_plot_sorted = swim_scores_to_plot.sort_values(by='date', ascending=False)
    ...
    for index, row in swim_scores_to_plot_sorted.iterrows():
        date_labels.append(row['date'])
        score_labels.append(row['sTSS'])
        distance_labels.append(row['distance'])
        duration_labels.append(row['duration'])
        pace_labels.append(row['average pace'])

    labels = ['<p style="font-family: Arial;font-size: 12px">{title1}<br>sTSS: {title2}<br>{third}<br>{fourth}<br>{fifth}</p>'.format(
            title1=date_labels[x].strftime("%d/%m/%Y"), title2=str(score_labels[x]), third=distance_labels[x], fourth=time.strftime('%H:%M:%S', time.gmtime(duration_labels[x])), fifth=pace_labels[x]) for x in range(len(swim_scores_to_plot.index))]
        ...
        if count==1:
            for date1 in (single_date - timedelta(n) for n in range(42)):
                ...
                if date1 in (single_date - timedelta(n) for n in range(7)):
                    for index, row in swim_scores_df.iterrows():
                        #print(type(row['date']))
                        #print(type(date1))
                        if row['date'].date() == date1:
                            days_since = (single_date - date1).days
                            form_r += row['sTSS']*weighting[days_since]
                            form_w += row['sTSS']
                            day_score += row['sTSS']
                else:
                    for index, row in swim_scores_df.iterrows():
    ...
    fig, ax = plt.subplots()
    points = ax.scatter(x=swim_scores_to_plot_sorted['date'], y=swim_scores_to_plot_sorted['sTSS'], s=2, c='r')
